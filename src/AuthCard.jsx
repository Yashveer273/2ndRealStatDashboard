...
const AuthCard = ({ onLoginSuccess }) => {
  ...
  const handleLogin = async () => {
    ...
    if (res.success) {
      ...
      localStorage.setItem("2ndrealStateLoggedUser", JSON.stringify({ phone, type: loginType }));
  ...
  const handleForgetPassword = async () => {
    ...
    try {
      ...
      if (res.success) {
        ...
        localStorage.setItem("2ndrealStateLoggedUser", JSON.stringify({ phone, type: "admin" }));
        resetAll();
        setMode("login");
      } else {
